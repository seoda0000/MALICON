...
const VideoAvatarSection: React.FC<{ played: number; emotionLog: any }> = (
  ...
  console.log("VideoAvatarSection", props.emotionLog);
  ...
  useEffect(() => {
    props.emotionLog.map((emotion: any) => {
      ...
      // 타임라인 생성
      let time = Math.round(emotion.timestamp / 1000);
    ...
    console.log("emotionLogChange", emotionLogChange);
  }, []);

  const [emotionLog, setEmotionLog] = useState(emotionLogFirst);
  console.log("emotionLogChange2", emotionLogFirst);
  ...
  useEffect(() => {
    console.log("업데이트", emotionLog);
    timeline.map((item: any) => {
      if (item.timestamp === Math.round(props.played)) {
